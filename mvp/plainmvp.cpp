...
bool PlainBlockMatVecProduct(DoContext* ctx, const uint32_t* __restrict__ mat,    // matrix: size n × m, block-wise, row-major
    const uint32_t* __restrict__ vec,    // vector: length m
    uint32_t*       __restrict__ result, // output: size s × n, row-major blocks
    uint32_t n, uint32_t m,
    uint32_t s, uint32_t p
    ...
    for (uint32_t blk = 0; blk < s; ++blk) {
        ...
        if (!PlainMatVecProduct(ctx, mat_blk, vec_blk, result_blk, n, b, p)) {
            return false;
        }
    }
    return true;
...
bool PlainMatVecProductExt(
    ...
    vec += vo;
    result += ro;
    for (int s = 0; s < steps; s++, mat += ms, vec += vs, result += rs) {
        if (!PlainMatVecProduct(ctx, mat, vec, result, n, m, p)) {
            return false;
        }
    }
    return true;
